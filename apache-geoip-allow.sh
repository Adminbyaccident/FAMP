...
#
# PURPOSE: This script installs and configures Apache HTTP to allow connections from a specific country/countries on a FreeBSD system.
# 		   Notice only the specified countries are allowed and ALL the rest are not!
#
...
# Example: USA='SetEnvIf MM_COUNTRY_CODE ^(US) CountryList'
# Uncomment and add as many lines below as you need.
# Country_1='SetEnvIf MM_COUNTRY_CODE ^(US) CountryList'
# Country_2='SetEnvIf MM_COUNTRY_CODE ^(UK) CountryList'
...
echo "
Geolocation allow has been installed and configured for $Country_1 , $Country_2"
